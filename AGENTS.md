...
- Brand/content: read `.claude/rules/brand-voice.md`, `content-pipeline.md` and `content-formats.md` when relevant. The September 9 authority section in `brand-voice.md` supersedes older universal price/rush/design/timing mandates in those content checklists. Read `docs/social/BRANDING-STANDARD.md` for the scoped illustrative-hero logo correction. Customer communication and public publishing require explicit authorization; research alone is not consent to send.
