...
CREATE TABLE public.social_campaigns(id uuid PRIMARY KEY,slug text UNIQUE,name text);
CREATE TABLE public.social_accounts(id uuid PRIMARY KEY);
CREATE TABLE public.social_posts(id uuid PRIMARY KEY,campaign_id uuid REFERENCES public.social_campaigns(id) ON DELETE SET NULL,status text,approval_hash text,updated_at timestamptz,schedule_time timestamptz);
CREATE TABLE public.social_post_results(id uuid PRIMARY KEY,post_id uuid REFERENCES public.social_posts(id) ON DELETE CASCADE);
...
INSERT INTO public.social_campaigns(id,business_id,slug,name) VALUES('b0000000-0000-4000-8000-000000000001','00000000-0000-4000-8000-000000000002','monthly','Synthetic B'),('b0000000-0000-4000-8000-000000000002','00000000-0000-4000-8000-000000000001','monthly','Synthetic A');
DO $$ DECLARE t text; r text; BEGIN
 ...
 EXCEPTION WHEN foreign_key_violation THEN NULL; END;
 BEGIN
  INSERT INTO public.social_campaigns(id,business_id,slug) VALUES(gen_random_uuid(),'00000000-0000-4000-8000-000000000001','monthly');
  RAISE EXCEPTION 'Same business duplicate slug accepted';
 EXCEPTION WHEN unique_violation THEN NULL; END;
 INSERT INTO public.social_posts(id,business_id,campaign_id) VALUES('a0000000-0000-4000-8000-000000000002','00000000-0000-4000-8000-000000000001','b0000000-0000-4000-8000-000000000002');
 INSERT INTO public.social_post_results(id,post_id,business_id) VALUES('c0000000-0000-4000-8000-000000000001','a0000000-0000-4000-8000-000000000002','00000000-0000-4000-8000-000000000001');
 DELETE FROM public.social_campaigns WHERE id='b0000000-0000-4000-8000-000000000002';
 IF NOT EXISTS(SELECT FROM public.social_posts WHERE id='a0000000-0000-4000-8000-000000000002' AND campaign_id IS NULL AND business_id='00000000-0000-4000-8000-000000000001') THEN RAISE EXCEPTION 'Campaign SET NULL lifecycle lost'; END IF;
 DELETE FROM public.social_posts WHERE id='a0000000-0000-4000-8000-000000000002';
 IF EXISTS(SELECT FROM public.social_post_results WHERE id='c0000000-0000-4000-8000-000000000001') THEN RAISE EXCEPTION 'Receipt CASCADE lifecycle lost'; END IF;
