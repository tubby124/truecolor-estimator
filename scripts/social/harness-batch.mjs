...
export async function buildHarnessBatch(input, assetDirectory, outputDirectory) {
  ...
  // Validate and render every source before writing any deliverable.
  const media = [];
  for (const creative of batch.plan.creatives) {
    const source = await readFile(join(assetDirectory, creative.imageFilename));
    if (!source.length || source.length > 30 * 1024 * 1024) throw new Error(`${creative.imageFilename}: source must be 1 byte to 30 MB.`);
    if (sha(source) !== creative.imageSha256) throw new Error(`${creative.imageFilename}: source bytes changed.`);
    const prepared = await prepareMonthlyMedia(source);
    media.push({ creative, source, prepared });
  }
  ...
  const sidecar = { schemaVersion: 1, kind: 'truecolor-harness-lineage', packageId: batch.packageId, businessKey: batch.businessKey,
    ...
    schedulerEnrollment: 'not_enrolled', learningContext,
    creatives: media.map(({ creative, prepared }) => ({ ...batch.lineage.find(row => row.creativeId === creative.id), sourceSha256: creative.imageSha256, expectedUploadSha256: sha(prepared.buffer), expectedUploadWidth: prepared.width, expectedUploadHeight: prepared.height, scheduleTime: creative.scheduleTime, channels: creative.channels })) };
