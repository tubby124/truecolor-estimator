...
export function checkedPreparationPolicy(input, batch) {
  ...
  if (!Array.isArray(policy.applied_decision_ids) || !policy.applied_decision_ids.length || new Set(policy.applied_decision_ids).size !== policy.applied_decision_ids.length || policy.applied_decision_ids.some(id=>!context.consultedDecisionIds.includes(id)) || !/^[a-f0-9]{64}$/.test(policy.binding_sha256 ?? '')) throw new Error('Preparation policy decision evidence mismatch.');
