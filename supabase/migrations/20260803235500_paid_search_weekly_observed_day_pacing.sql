-- Keep partial-week pacing honest by dividing spend by published campaign days,
-- not by every calendar day in the week. The published interface is unchanged.
...
WITH weekly_campaign AS (
  ...
  SELECT
    weekly_campaign.*,
    GREATEST(1, observed_days) AS campaign_observed_days,
    GREATEST(
      1,
      max(observed_days) OVER (
        PARTITION BY google_ads_customer_id, week_start
      )
    ) AS pilot_observed_days,
...
COMMENT ON FUNCTION public.google_ads_published_weekly_campaign_rows() IS
  'Service-role-only weekly paid-search decision rows. Daily pace uses observed published campaign days, including honest partial-week pacing; recommendations remain advisory and read-only.';
...
BEGIN
  IF has_table_privilege(
    ...
    RAISE EXCEPTION
      'Paid-search pacing hardening failed: staging SELECT exposed';
  END IF;
  ...
  IF NOT has_function_privilege(
    ...
    RAISE EXCEPTION
      'Paid-search pacing hardening failed: published function unavailable';
  END IF;
END;
$$;
