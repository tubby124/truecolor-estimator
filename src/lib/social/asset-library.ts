...
export type LibraryCatalog = { schemaVersion: 1; businessId: "truecolor"; collectedAt: string; assets: LibraryAsset[] };
...
export function parseLibraryCatalog(value: unknown): LibraryCatalog {
  ...
  if (!c || c.schemaVersion !== 1 || c.businessId !== "truecolor" || typeof c.collectedAt !== "string" || !Number.isFinite(Date.parse(c.collectedAt)) || !Array.isArray(c.assets) || c.assets.length > 2000) throw new Error("Invalid library catalog");
  ...
  for (const a of c.assets) {
    if (!a || ["id", "sha256", "storagePath", "filename", "title", "description", "alt", "category", "kind"].some(k => typeof a[k as keyof LibraryAsset] !== "string" || (a[k as keyof LibraryAsset] as string).length > 4000)
      ...
      || !/^originals\/[a-zA-Z0-9_./-]+$/.test(a.storagePath) || a.storagePath.split("/").some(s => !s || s === "." || s === "..")
  ...
  return { schemaVersion: 1, businessId: "truecolor", collectedAt: c.collectedAt, assets: c.assets.map(asset => Object.fromEntries(keys.map(key => [key, asset[key]])) as LibraryAsset) };
}
