...
import { generationServiceFacts, productHashtagDefaults, TRUE_COLOR_BUSINESS_PROFILE } from './business-profile';
...
export function assemblePrompt(input: GenerationInput, facts: ProductFacts | null, observation: GenerationObservation | null, recentSummary: string, tags: string[]) {
  ...
  return {
    ...
    context: JSON.stringify({
      ...
      postingCadence: TRUE_COLOR_BUSINESS_PROFILE.postingCadence,
      weeklyImageMix: TRUE_COLOR_BUSINESS_PROFILE.weeklyImageMix,
      observations: observation, hashtagRules,
      hashtagPolicy: hasMeta ? TRUE_COLOR_BUSINESS_PROFILE.hashtagPolicy : null,
      genericHashtagDefaults: hasMeta ? productHashtagDefaults(input.productSlug) : [],
      hashtagCandidates: hasMeta ? [...new Set([...TRUE_COLOR_BUSINESS_PROFILE.coreMetaHashtags, ...productHashtagDefaults(input.productSlug), ...tags])].slice(0, 12) : [],
    }),
  };
}
