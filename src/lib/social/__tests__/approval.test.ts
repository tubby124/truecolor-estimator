...
function deliveryDb(initial: SocialPost) {
  let row = { ...initial };
  const events: string[] = [];
  ...
  return {
    ...
    from(table: string) {
      ...
      const chain = {
        ...
        async upsert(receipt: unknown) { receipts.push(receipt); events.push('receipt'); return { error: null }; },
      };
      return chain;
    },
  };
...
describe('approved delivery positive path', () => {
  ...
  afterEach(() => { vi.unstubAllEnvs(); vi.unstubAllGlobals(); vi.useRealTimers(); });
  it('claims once, validates exact bytes, persists receipt, posts, and rejects duplicate delivery', async () => {
    const bytes = await sharp({ create: { width: 1080, height: 1080, channels: 3, background: '#fff' } }).jpeg().toBuffer();
    const post = { ...approved(), approved_media_sha256: createHash('sha256').update(bytes).digest('hex') };
    ...
    publish.mockImplementation(async () => {
      ...
      return { attempted: true, results: [{ platform: 'instagram', status: 'published', submissionId: 'synthetic-media', publicUrl: 'https://example.test/synthetic-post' }] };
    });
    const result = await dispatchApprovedPost(db as never, post);
    expect(result.status).toBe(200);
    expect(db.events).toEqual(['posting', 'provider', 'receipt', 'posted']);
    expect(db.receipts).toEqual([expect.objectContaining({ post_id: post.id, platform: 'instagram', status: 'published', blotato_submission_id: 'synthetic-media', public_url: 'https://example.test/synthetic-post' })]);
