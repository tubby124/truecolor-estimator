...
export class GenerationRequestError extends Error { constructor(public status: number, message: string) { super(message); } }
...
export async function generateCaptions(input: GenerationInput, businessId?: string): Promise<GenerationResponse> {
  ...
  if (!response.ok) throw new GenerationRequestError(response.status, data.error ?? 'Caption generation unavailable. Keep this request ID for recovery.');
