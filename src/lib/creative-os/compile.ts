import { ContractError, digest, validateBriefInput, validateBundle, validateClaims, validateSourceRefs, type ClaimRef, type Bundle, type BriefInput, type Configuration, type OfferMode, type SourceRef } from './contracts';
...
export interface FactsResolution {
  ...
  sourceRefs: SourceRef[]; claims: ClaimRef[]; requiredVisibleText: string[]; cta: { url: string; instruction: string; preselectionVerified: false };
  quote: null | {
    currency: string; amountMinor: number; rawSubtotalMinor: number; minimumAdjustmentMinor: number;
...
export interface LocalFactsAdapter { brandKey: string; validateConfiguration(input: unknown): Configuration; resolve(request: BriefInput): FactsResolution }
export interface CompiledBrief {
  schemaVersion: 2; kind: 'creative_brief'; id: string; brandKey: string; status: 'draft'; usageScope: 'fixture_only' | 'review_candidate';
  createdAt: string; kitRef: { id: string; revision: number; digest: string }; recipeRef: { id: string; revision: number; digest: string };
  proofRefs: { id: string; revision: number; digest: string; proofKind: string; disclosure: string; usageScope: string; assetId: string; sha256: string; sourceRefId: string; sourceRefs: SourceRef[] }[];
  claimRefs: ClaimRef[];
  headlineDirection: string; requiredVisibleText: string[]; captionGuidance: string; altTextGuidance: string;
  layoutInstructions: string[]; contactPanel: Partial<Bundle['kit']['identity']>; forbiddenClaims: string[];
...
export function compileBrief(input: unknown, rawBundle: unknown, adapter: LocalFactsAdapter): CompiledBrief {
  ...
  if (request.mode === 'exact_configuration' && digest(adapter.validateConfiguration(structuredClone(request.configuration))) !== digest(request.configuration)) throw new ContractError('Adapter changed canonical configuration');
  const facts = adapter.resolve(structuredClone(request));
  ...
  validateSourceRefs(facts.sourceRefs);
  if (facts.cta.preselectionVerified !== false) throw new ContractError('Unsupported preselected action claim');
  validateClaims(facts.claims,request.productSlug,facts.sourceRefs);
  const selectedClaims = recipe.claimIds.map(id => { const claim = facts.claims.find(c => c.id === id); if (!claim) throw new ContractError('Missing recipe claim reference'); return claim; });
  for (const text of recipe.requiredVisibleText) if (!selectedClaims.some(c => c.text === text)) throw new ContractError('Unattributed recipe commercial claim');
  for (const text of [...facts.requiredVisibleText,facts.cta.instruction]) if (!facts.claims.some(c => c.text === text)) throw new ContractError('Unattributed adapter commercial claim');
  ...
  if (request.mode === 'exact_configuration') {
    ...
    if (!/^[A-Z]{3}$/.test(facts.quote.currency) || !/^[a-f0-9]{64}$/.test(facts.quote.factFingerprint) || !Number.isSafeInteger(facts.quote.amountMinor) || facts.quote.amountMinor <= 0 || facts.quote.costMinor !== null || facts.quote.margin !== null) throw new ContractError('Invalid quote or unsupported cost claim');
    if (![facts.quote.rawSubtotalMinor,facts.quote.minimumAdjustmentMinor].every(n => Number.isSafeInteger(n) && n >= 0) || facts.quote.rawSubtotalMinor + facts.quote.minimumAdjustmentMinor !== facts.quote.amountMinor || (facts.quote.minimumAdjustmentMinor > 0 && !facts.quote.minimumDisclosure?.trim())) throw new ContractError('Invalid quote reconciliation or minimum disclosure');
  } else {
  ...
  const lane = recipe.visualLane;
  const panel = recipe.contactPanel;
  ...
  for (const text of [...bundle.kit.voice,...bundle.kit.visualConstraints,bundle.kit.logoPolicy.instructions,bundle.kit.actionPolicy.instructions,lane.instructions,panel.instructions,proof.disclosure]) {
  ...
  const usageScope = bundle.kit.usageScope === 'fixture_only' || bundle.proofs.some(p => p.usageScope === 'fixture_only') ? 'fixture_only' : 'review_candidate';
  ...
  return {
    schemaVersion: 2, kind: 'creative_brief', id: request.id, brandKey: request.brandKey, status: 'draft', usageScope,
    ...
    proofRefs: [{ id: proof.id, revision: proof.revision, digest: digest(proof), proofKind: proof.proofKind, disclosure: proof.disclosure, usageScope: proof.usageScope,assetId: proof.assetId,sha256: proof.sha256,sourceRefId: proof.sourceRefId,sourceRefs: proof.sourceRefs }],
    claimRefs: facts.claims.filter(c => selectedClaims.includes(c) || facts.requiredVisibleText.includes(c.text) || facts.cta.instruction === c.text),
    ...
    layoutInstructions: [...bundle.kit.visualConstraints,lane.instructions,panel.instructions,bundle.kit.actionPolicy.instructions,`Logo ${bundle.kit.logoPolicy.requirement}; illustrative overlay ${bundle.kit.logoPolicy.illustrativeOverlay}. ${bundle.kit.logoPolicy.instructions}`],
    contactPanel: Object.fromEntries(bundle.kit.actionPolicy.requiredIdentityFields.map(field => [field,bundle.kit.identity[field]])),
    ...
    diversitySignature: [recipe.productSlug,recipe.purpose,recipe.visualLane.id,recipe.contactPanel.id,...recipe.diversityTags],
