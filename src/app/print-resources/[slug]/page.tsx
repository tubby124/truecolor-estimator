      ...
      <main id="main-content">
        <header className="border-b border-[#e5e5e5] bg-white">
          <div className="mx-auto max-w-6xl px-6 pb-14 pt-8 sm:pb-18 sm:pt-10">
            <nav aria-label="Breadcrumb" className="mb-10 flex flex-wrap items-center gap-2 text-sm text-gray-500">
              <Link href="/" className="transition-colors hover:text-[#087fa1]">Home</Link>
              <span aria-hidden="true">/</span>
              <Link href="/resources" className="transition-colors hover:text-[#087fa1]">Resources</Link>
              <span aria-hidden="true">/</span>
            ...
            <div className="grid items-end gap-10 lg:grid-cols-[minmax(0,1fr)_21rem]">
              <div>
                <p className="mb-4 font-mono text-xs font-bold uppercase tracking-[0.22em] text-[#087fa1]">
                  {resource.eyebrow}
                </p>
              ...
              <aside aria-label="Order paths" className="border-l-4 border-[#16C2F3] bg-[#111111] p-6 text-white shadow-[8px_8px_0_#e5e5e5]">
                ...
                <div className="mt-4 space-y-3">
                  {resource.productLinks.slice(0, 2).map((link) => (
                    <Link key={link.href} href={link.href} className="group flex items-center justify-between gap-3 border-t border-white/20 pt-3 text-sm font-bold hover:text-[#16C2F3]">
        ...
        <div className="mx-auto grid max-w-6xl gap-12 px-6 py-12 lg:grid-cols-[minmax(0,1fr)_18rem] lg:py-16">
          <article className="min-w-0">
            {resource.download && (
              <section className="mb-12 border border-[#111111] bg-[#eefaff] p-6 sm:p-8">
                <div className="flex flex-col gap-6 sm:flex-row sm:items-center sm:justify-between">
                  <div className="flex gap-4">
                    <FileCheck2 aria-hidden="true" className="mt-1 h-7 w-7 shrink-0 text-[#087fa1]" />
                    <div>
              ...
              {resource.sections.map((section, index) => (
                <section key={section.heading} aria-labelledby={`section-${index}`}>
                  <div className="mb-5 flex items-start gap-4">
                    <span aria-hidden="true" className="mt-1 font-mono text-xs font-bold text-[#087fa1]">{String(index + 1).padStart(2, "0")}</span>
                    ...
                    {section.bullets && (
                      <ul className="space-y-3" role="list">
                        {section.bullets.map((bullet) => (
                          <li key={bullet} className="flex gap-3"><span aria-hidden="true" className="text-[#087fa1]">—</span><span>{bullet}</span></li>
                        ))}
                      </ul>
                    )}
                  </div>
          ...
          <aside aria-label="Matching products" className="lg:sticky lg:top-6 lg:self-start">
            ...
            <div className="mt-4 divide-y divide-[#e5e5e5] border-y border-[#e5e5e5] bg-white">
              {resource.productLinks.map((link) => (
                <Link key={link.href} href={link.href} className="group block p-5 transition-colors hover:bg-[#eefaff] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#16C2F3]">
                  <span className="flex items-start justify-between gap-3 font-bold leading-5 group-hover:text-[#087fa1]">
