...
const FEATURED_PAID_PRODUCT_SLUGS = [
  "coroplast-signs", "stickers", "vinyl-banners", "business-cards", "flyers", "retractable-banners",
  "acp-signs", "vehicle-magnets",
] as const;

const MORE_PAID_PRODUCT_SLUGS = [
  "photo-posters", "window-decals", "vinyl-lettering", "foamboard-displays", "postcards", "brochures",
] as const;

const buildPaidProducts = (slugs: readonly string[]) => slugs.map((slug) => {
...
export const FEATURED_PAID_PRODUCTS = buildPaidProducts(FEATURED_PAID_PRODUCT_SLUGS);
export const MORE_PAID_PRODUCTS = buildPaidProducts(MORE_PAID_PRODUCT_SLUGS);
export const PAID_PRODUCTS = [...FEATURED_PAID_PRODUCTS, ...MORE_PAID_PRODUCTS];
      ...
      <section className="overflow-hidden border-b border-gray-200 bg-[#f5f3ef] px-4 py-8 sm:px-6 sm:py-12">
        ...
        <div className="mx-auto mt-7 grid max-w-5xl grid-cols-2 gap-2 rounded-2xl border border-gray-200 bg-white p-3 shadow-sm sm:grid-cols-4 sm:gap-0 sm:p-0">
          ...
          <PaidCtaLink
            ...
            placement="hero_address"
            target="_blank"
            rel="noopener noreferrer"
            className="flex items-center gap-2 rounded-lg p-2.5 transition hover:bg-[#f5f3ef] hover:text-[#087c9d] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#16C2F3] sm:border-r sm:border-gray-100 sm:p-4"
          >
            <MapPin className="shrink-0 text-[#c92719]" size={20} aria-hidden="true" />
            <span className="text-xs font-bold text-[#1c1712] sm:text-sm">216 33rd St W, Saskatoon, SK</span>
          </PaidCtaLink>
          <div className="flex items-center gap-2 p-2.5 sm:border-r sm:border-gray-100 sm:p-4">
        ...
        <div className="mx-auto max-w-5xl">
          ...
          <div className="mt-7 grid grid-cols-2 gap-3 sm:grid-cols-3 sm:gap-4">
            <PaidProductListTracker
              products={FEATURED_PAID_PRODUCTS.map(({ slug, name }) => ({ slug, name }))}
              itemListName="Paid featured product chooser"
            />
            {FEATURED_PAID_PRODUCTS.map((product) => (
              <PaidProductLink
                key={product.href}
                href={product.href}
          ...
          <div className="mt-9 border-t border-gray-200 pt-8">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
              <div>
                <p className="text-xs font-bold uppercase tracking-[0.16em] text-[#00718f]">More things we print</p>
                <h3 className="mt-1 text-2xl font-black tracking-tight text-[#1c1712]">More products, same live pricing</h3>
              </div>
              <PaidCtaLink
                href="/products"
                action="browse_all_products"
                placement="product_catalogue"
                className="inline-flex min-h-11 items-center gap-1 self-start font-bold text-[#087c9d] underline decoration-[#16C2F3]/50 underline-offset-4 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#16C2F3] sm:self-auto"
              >
                See all products <ArrowRight size={16} aria-hidden="true" />
              </PaidCtaLink>
            </div>

            <div className="mt-5 grid grid-cols-2 gap-3 lg:grid-cols-3">
              <PaidProductListTracker
                products={MORE_PAID_PRODUCTS.map(({ slug, name }) => ({ slug, name }))}
                itemListName="Paid expanded product chooser"
                trackLandingView={false}
              />
              {MORE_PAID_PRODUCTS.map((product) => (
                <PaidProductLink
                  key={product.href}
                  href={product.href}
                  productSlug={product.slug}
                  ...
                  placement="product_catalogue"
                  itemListName="Paid expanded product chooser"
                  className="group flex min-h-24 items-center gap-3 rounded-2xl border border-gray-200 bg-[#f5f3ef] p-3 transition hover:-translate-y-0.5 hover:border-[#16C2F3] hover:bg-white hover:shadow-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#16C2F3] focus-visible:ring-offset-2"
                >
                  <span className="relative hidden h-16 w-16 shrink-0 overflow-hidden rounded-xl bg-gray-200 sm:block">
                    <Image
                      src={product.heroImage}
                      alt={`${product.name} available to order from True Color Display Printing`}
                      fill
                  ...
                  <span className="min-w-0 flex-1">
                    <span className="block text-sm font-black leading-tight text-[#1c1712]">{product.name}</span>
                    <span className="mt-1 block text-sm font-bold text-[#c92719]">From {product.fromPrice}</span>
                    <span className="mt-2 inline-flex items-center gap-1 text-xs font-bold text-[#087c9d]">
                      Price &amp; order <ArrowRight size={14} aria-hidden="true" />
                    </span>
                  </span>
                </PaidProductLink>
              ))}
          ...
          <div className="mt-7 grid gap-3 sm:grid-cols-3">
            {REAL_WORK.map((photo) => (
              <PaidProductLink
                ...
                placement="gallery"
                itemListName="Paid real work gallery"
      ...
      <section className="px-4 pb-12 sm:px-6 sm:pb-16" aria-labelledby="closing-cta-heading">
        <div className="mx-auto rounded-3xl bg-[#c92719] p-7 text-white shadow-lg sm:p-10">
          ...
          <div className="mt-6 flex flex-col gap-3 sm:flex-row">
            ...
            <PaidCtaLink href="/quote" action="custom_quote" placement="closing" className="inline-flex min-h-12 items-center justify-center rounded-xl border border-white/60 px-6 font-black text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-offset-2 focus-visible:ring-offset-[#c92719]">
              Request My Quote
            </PaidCtaLink>
          </div>
        </div>
