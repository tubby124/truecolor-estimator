 ...
 * layers). PST stays exempt on kind="fee" lines. See payment-tax.md.
 */
...
describe("applyOverrideTotal — exact tax-consistent back-solve", () => {
  const cases: Array<{ name: string; items: OrderItemInput[]; target: number }> = [
    ...
    { name: "last line is a fee → $99.99", items: [product(80), fee(35)], target: 99.99 },
    { name: "all fees (PST must be $0) → $77.77", items: [fee(20), fee(50)], target: 77.77 },
  ...
  for (const { name, items, target } of cases) {
    it(`${name} reconciles to the cent with a clean tax split`, () => {
      ...
      // Tax split is exactly GST 5% of subtotal + PST 6% of non-fee subtotal —
      ...
      const pstable = scaled
        .filter((i) => i.kind !== "fee")
        .reduce((s, i) => s + Math.round(i.amount * 100), 0);
      expect(breakdown.pstCents).toBe(Math.round(pstable * 0.06));
    });
  }
  ...
  it("leaves amounts untouched when no override is given", () => {
    ...
    // 45*1.05 + 45*0.06 + 35*1.05 = 47.25+2.70+36.75
    expect(breakdown.totalCents).toBe(8670);
  });

  it("preserves PST exemption: all-fee order collects zero PST", () => {
    const { breakdown } = applyOverrideTotal([fee(20), fee(50)], 77.77);
    expect(breakdown.pstCents).toBe(0);
