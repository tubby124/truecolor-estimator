...
let status: string, previous: unknown, historyError: unknown;
...
beforeEach(() => {
  vi.clearAllMocks(); status = "payment_received"; previous = null; historyError = null;
  m.auth.mockResolvedValue({ email: "staff@example.test" });
  m.from.mockImplementation(table => table === "email_log" ? chain(previous, historyError) : chain({ id: "order", order_number: "TC-TEST", status, subtotal: 100, gst: 5, pst: 6, total: 111, payment_method: "clover_card", customers: { name: "Test", email: "test@example.test" }, order_items: [] }));
});
