  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      try {
        ...
        // Same helper + payload shape as ProductConfigurator.tsx:223. item_id
        // mirrors that call's `product.material_code` (the catalog base code,
        // not the per-selection material) so events stay comparable across the
        // ProductPageClient flag flip. Customer surfaces only — the staff
        // quoting tool must not pollute the funnel.
        if (mode === "customer") {
          trackPriceCalculated({
            item_id: category === "STICKER" ? STICKER_CONTENT.materialCode : category,
