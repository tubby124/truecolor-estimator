...
export function MonthlyBatchScheduler() {
  ...
  return <main className="mx-auto max-w-4xl space-y-6 p-6">
    ...
    {session.creatives.map((c, i) => <article key={c.id} className="space-y-3 rounded-xl border p-4">
      ...
      {c.usage && <p className="text-xs">Usage for this job: {c.usage.calls} provider calls · {c.usage.promptTokens ?? 'unknown'} input tokens · {c.usage.completionTokens ?? 'unknown'} output tokens · {c.usage.costUsd === null ? 'cost unavailable' : `US$${c.usage.costUsd.toFixed(4)}`}</p>}
