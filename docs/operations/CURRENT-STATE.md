...
[Connected preparation](../social/CONNECTED-PREPARATION.md) adds an explicit learning-to-settings compiler and a one-command runner consuming the companion PR77 bridge. Current ledger→typed white-logo policy→raw render→final uploader preview succeeded for three creatives with actual file/caption/time verification. Forty Python tests and independent review pass. Desktop/phone final preview checked. Exact same-package browser/queue rehearsal passed with three creatives/six mocked ready destinations; parent verified matching plan SHA across preparation and browser receipts. No live upload/approval/enrollment/publication. Captions remain supplied drafts, not automatically rewritten from free-form learning. This replaces passive feedback attachment for the implemented rendering rule, without claiming an unattended monthly system.
