...
| Merchant commerce / commerce task | Sep 7: Google rejected the product-page experience. PRs #62–63 are merged and successfully deployed; the exact example now explicitly matches local `out_of_stock` truth and exposes the sole Saskatoon pickup store, address, price and pickup-later timing. Live feeds remain 18 primary / 18 local rows. | Owner-confirm the enabled Merchant Save/resubmission, then read Google's new review, pickup-later, inventory-validation and source-specific eligibility results. Retain 25 legacy manual products until replacement serving evidence. Do not widen to Ads or invented stock/shipping. |
