...
Owner instruction September 9: apply the `seo-images` skill to every subsequent image task and reread these rules from current GitHub main before release. Its review covers explicit descriptive alt (10–125 characters), actual file bytes/MIME, descriptive filenames, responsive srcset/sizes and DPR, LCP loading/fetch priority, below-fold decoding/loading, reserved layout space and cache behavior. Record an image audit alongside the before/after evidence. These image checks do not authorize a metadata, schema, sitemap or indexed-page rewrite. Follow this runbook's channel gates and project tolerances when older generic skills/rules conflict; missing account evidence never becomes a performance claim.
...
Use existing [approval](../social/APPROVAL-PILOT.md) and [portable-system](../social/PORTABLE-SOCIAL-SYSTEM.md) contracts; do not build a duplicate queue. For orderable sales posts, bind product, exact configuration/quantity/current price/tax treatment and matching order URL. Quote-only and coming-soon products need different CTAs. A rectangular decal price must not be attached to a pictured custom-cut configuration. Preserve owner-approved photo treatment; a price-bearing caption does not automatically require a new poster-style image.
