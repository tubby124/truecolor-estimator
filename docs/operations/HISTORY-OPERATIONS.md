...
September's staff-copyable pay link exposed why "the order total" and "what the customer owes" are different facts. A partial Clover capture records an `order_payments` row, sends staff Telegram, and leaves the order in `pending_payment` with `orders.total` unchanged — so any code that mints or validates a `/pay` amount from `orders.total` silently overcharges a customer who already paid a deposit, and any code that mints the ledger balance was rejected as a stale link. Treat `remainingBalanceCents(orderTotal, ledger)` as the single definition of what a link charges, and keep it on both sides: the gateway that accepts a link and every path that mints one. A ledger read that fails must fail closed rather than fall back to the total. Two related traps remain worth knowing: order-creation mints are legitimately total-based only because no payment can exist yet, and `reserve_order_checkout` can resume a Clover session created up to 16 minutes earlier without re-checking the amount. Before adding any new path that hands a customer a pay link, reuse `resolveOrderPayLink()` instead of encoding a token by hand.
