...
## September morning schedule and scoped runtime — September 6, 2026
...
# September VPS cutover completed — September 7, 01:12 UTC

Fresh provider/database reconciliation at 01:07 UTC confirmed all six original Sunday deliveries posted, each with exactly one published receipt and one exact-caption provider match. The final 7 p.m. Regina pair is live on [Facebook](https://www.facebook.com/122234777786295517/posts/122234883548295517) and [Instagram](https://www.instagram.com/p/Dc94Ybbm7k8/). No delivery was retried during this continuation. Independent month readback still found 48 ready, approved destinations, 24 unchanged images and zero calendar/caption/media validation failures.

The exact September runner and separate read-only Telegram monitor are now installed and active on the VPS. Installed script/unit hashes match accepted source. A protected backup retains the prior pilot runner, configuration and state; checksums confirmed original pilot journals and the three credentials remained unchanged. The monitor was initialized once, and its marker/outbox remain durable. No setup diagnostic was resent.

Read-only scoped app checks and the hardened installed-runner preflight passed. The existing sole posting timer now runs the exact approved September scope. The independent monitor has its own two-minute timer. At 01:12 UTC both scheduled timers had run successfully; the posting heartbeat had exit code zero, its journal was waiting, and monitoring reported no problems or queued notifications. No second posting timer was added. The normal VPS posting schedule no longer depends on the Mac staying awake.

The first new September pair remains future at September 7, 9 a.m. Regina / 15:00 UTC. Its actual provider links and duplicate check are the next verification; active timers and approvals do not prove that future publication. Whole-VPS or monitor-host failure remains outside this same-host monitor's coverage. October remains excluded.

The API-first efficiency guide is merged in [PR59](https://github.com/tubby124/truecolor-estimator/pull/59) after exact-head application/browser, SQL and security checks passed. AISA's existing collaboration learnings update is merged in PR18 after validation/security passed; the blocked Vercel preview is not an AISA deployment receipt. Do not repeat those captures.
