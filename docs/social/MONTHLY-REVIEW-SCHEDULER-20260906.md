...
## Full-month import and delivery progress

The [September6 full-month extension](FULL-MONTH-WORKFLOW-20260906.md) adds local versioned-package import with source-file hash verification, opt-in non-cropping JPEG framing, and independent read-only delivery progress in the existing exact review. The original chunk-save, destination approvals and activation gates above remain the authority. GPT concept/holiday/audience planning is demonstrated privately; hosted image production is a separate remaining implementation.
