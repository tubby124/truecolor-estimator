...
Optional learningContext is the learning task's `social-draft-learning-context-v1` JSON. The bridge verifies its canonical digest, exact business/target draft and recipe/version mapping, then retains consulted decision IDs and the context hash. This is provenance only, not automatic recipe application, authenticated owner acceptance or permission to publish. Broader source-package lessons remain visibly broader than exact creative approval. Keep the original learning artifact private with the input.
...
- `month-plan.json` and matching originals in `images/`, directly accepted by the current monthly importer.
...
The tests exercise current parser/API contracts, immutable creative identity, exact copy/time transfer, image hash failure, lineage/clearance holds, same-day separate times, conflicting slots, private output and consulted context integrity. A Chromium rehearsal with local fixture authentication and mocked staff endpoints exercises file selection, preparation, save and reload/resume without duplicate save or approval/dispatch. A separate run can use `HARNESS_REHEARSAL_PLAN` pointing to the actual private month-plan; staff endpoints remain mocked and all real service access stays excluded.
