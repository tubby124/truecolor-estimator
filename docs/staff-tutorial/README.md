...
How to quote any job inside the website, even when the estimator can't handle it. Updated 2026-05-11 for the new modal.
...
> **Note on screenshots below:** PNGs are auto-generated by the playwright spec on each run. They will lag behind UI changes by one CI cycle. The text descriptions below are authoritative — when the screenshots show different UI than what you see in the modal, trust the text.
...
Each item card uses the same shape Albert uses in his quotes — **Material / Sides / Size / Process** as separate fields. Every field accepts a click-a-chip shortcut OR typed text.
...
The customer will see this in the email as a clean spec block (same format Albert has used for years):
...
- **Wave Invoice** — customer gets the Wave invoice email with pay-online + e-transfer instructions. **Wave PDF renders Albert's spec block too** — customers see the same multi-line format you set up in the modal.
...
You can also **"Reorder"** from a customer's past order — the chips and price refill from the previous quote so you don't have to retype.
...
| **Anything Albert quotes by email** — Plinko stickers, shelf talkers, yard signs, removal jobs, custom shapes | | ✅ |
...
- **Materials list** has the 16 most common Albert quotes pre-filled. If you need something else (uncommon paper weight, exotic vinyl), just type it in.
- **Process** is multi-select — click "Gloss lamination" AND "Die cut" AND "Grommets" and the field becomes "Gloss lamination / Die cut / Grommets" exactly the way Albert writes it.
- **"Other" is gone as a fallback.** Type whatever the product actually is — the combobox accepts free text. Customer sees the real product name on the invoice.
- **Stuck on a chip you don't see?** Type in the input field above the chips. Whatever you type goes through.

*Manually updated: 2026-05-11 (text-first). Screenshots regenerate when the playwright spec runs — they may briefly lag behind UI changes.*
